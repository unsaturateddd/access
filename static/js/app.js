...
const SETTINGS = {
  balance: Number(localStorage.getItem('1win_balance')) || 5000,
  amount: 50,
  mines: 3,
  cellOpened: 0,
  currency: 'RUB',
...
window.onresize = () => {
  ...
  const socket = io("wss://prod-rnd-websocket-php.100hp.app/mines", {
    transports: ["websocket"], // Используем только WebSocket
    query: {
        Authorization: "3ff6be4f-7f91-53b1-a12b-2d97166375c6", // Ваш токен
        Language: "en",
        xorigin: "1waaa.top"
    }
});
...
// 🔹 Получение событий о ставках (mines_live_bet_event)
...
const calculateCoefficient = (bet, resultBalance) => {
    return (resultBalance / bet).toFixed(2);  // Округляем до 2 знаков после запятой
};

function addBetToHistory(data) {
const truncatedUserName = data.user.length > 15 ? data.user.slice(0, 15) + '...' : data.user;
    ...
    listWrapper.prepend(`
        <div class="history-element-container">
            <div class="collapsable-header history-live">
                ...
                <div class="coef">x${calculateCoefficient(data.bet, data.resultBalance)}</div>
    ...
    // Ограничиваем список 20 элементами (удаляем последние)
